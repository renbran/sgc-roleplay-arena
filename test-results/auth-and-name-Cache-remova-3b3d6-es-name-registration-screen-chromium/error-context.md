...
- Name: auth-and-name.spec.ts >> Cache removal — auth v2 key enforcement >> authenticated user with no name sees name registration screen
- Location: e2e\auth-and-name.spec.ts:103:7
...
Locator: getByPlaceholder('Your full name')
Expected: visible
Timeout: 3000ms
...
Call log:
  ...
  - waiting for getByPlaceholder('Your full name')
